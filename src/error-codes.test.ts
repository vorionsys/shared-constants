...
describe('formatErrorMessage', () => {
  it('replaces template parameters', () => {
    const result = formatErrorMessage(VALIDATION_ERRORS.MISSING_REQUIRED_FIELD, {
      field: 'email',
    });
...
describe('createErrorResponse', () => {
  it('creates response without params', () => {
    ...
    expect(response.error.docsUrl).toBe('https://cognigate.dev/docs/authentication');
  });

  it('creates response with params', () => {
    const response = createErrorResponse(VALIDATION_ERRORS.MISSING_REQUIRED_FIELD, {
      field: 'name',
    });
  ...
  it('includes requestId when provided', () => {
    const response = createErrorResponse(AUTH_ERRORS.MISSING_API_KEY, undefined, 'req-123');
